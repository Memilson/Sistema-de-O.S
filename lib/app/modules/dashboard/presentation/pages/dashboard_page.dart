...
class _DashboardPageState extends State<DashboardPage> {
  ...
  double _totalFor(_OrderStatus? status) {
    return _orders
        ...
        .fold(0, (total, order) => total + order.value);
  }
  ...
  Future<void> _loadOrders() async {
    ...
    try {
      ...
      if (!mounted) return;
      setState(() {
        _orders = ordens.map(_DashboardOrder.fromOrdemServico).toList();
      ...
      setState(() {
        _loading = false;
        _error = 'Não foi possível carregar as ordens de serviço.';
      });
    }
  }
  ...
  Future<void> _abrirWhatsapp() async {
    ...
    if (!opened && mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Configure SUPPORT_WHATSAPP no .env.')),
      );
    }
  }
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [
          IconButton(
            tooltip: 'Clientes',
            onPressed: () => _openAndReload(AppRoutes.clientes),
            icon: const Icon(AppIcons.customers),
          ),
          IconButton(
            tooltip: 'Ordens',
            onPressed: () => _openAndReload(AppRoutes.ordens),
            icon: const Icon(AppIcons.orders),
          ),
          IconButton(
            tooltip: 'Nova O.S.',
            onPressed: () => _openAndReload(AppRoutes.novaOs),
        ...
        child: SingleChildScrollView(
          ...
          child: Column(
            ...
            children: [
              ...
              Text(
                _syncPending == 0
                    ? 'Resumo financeiro das ordens cadastradas no Supabase.'
                ...
                LayoutBuilder(
                  builder: (context, constraints) {
                    ...
                    return GridView.count(
                      ...
                      childAspectRatio:
                          constraints.maxWidth >= 720 ? 1.25 : 1.1,
                      crossAxisSpacing: 12,
                      mainAxisSpacing: 12,
                      shrinkWrap: true,
                      ...
                      children: [
                        ...
                        _KpiCard(
                          title: 'Em aberto',
                          count: _countFor(_OrderStatus.open),
                          value: _totalFor(_OrderStatus.open),
                          icon: AppIcons.pending,
                          color: Colors.amber.shade800,
                          selected: _selectedStatus == _OrderStatus.open,
                          formatter: _currencyFormat,
                          onTap: () => _selectStatus(_OrderStatus.open),
                        ),
                        _KpiCard(
                          title: 'Em execução',
                          count: _countFor(_OrderStatus.running),
                          value: _totalFor(_OrderStatus.running),
                          icon: AppIcons.tools,
                          color: Colors.deepPurple,
                          selected: _selectedStatus == _OrderStatus.running,
                          formatter: _currencyFormat,
                          onTap: () => _selectStatus(_OrderStatus.running),
                        ),
                        _KpiCard(
                          title: 'Executada',
                          count: _countFor(_OrderStatus.done),
                          value: _totalFor(_OrderStatus.done),
                          icon: AppIcons.checkCircle,
                          color: Colors.green,
                          selected: _selectedStatus == _OrderStatus.done,
                          formatter: _currencyFormat,
                          onTap: () => _selectStatus(_OrderStatus.done),
                        ),
                      ],
                    );
                  },
                    ...
                    Expanded(
                      child: Text(
                        _selectedStatus == null
                            ? 'Ordens recentes'
                            : 'Ordens ${_selectedStatus!.label.toLowerCase()}',
    ...
    return Card(
      ...
      child: ListTile(
        leading: CircleAvatar(
          backgroundColor: order.status.color.withAlpha(28),
...
enum _OrderStatus {
  open('Em aberto', AppIcons.pending, Colors.amber),
  running('Em execução', AppIcons.tools, Colors.deepPurple),
  done('Executada', AppIcons.checkCircle, Colors.green);
