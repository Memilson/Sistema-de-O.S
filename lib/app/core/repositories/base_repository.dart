...
abstract class BaseRepository<T extends BaseModel> {
  ...
  Future<List<T>> listar({
    ...
    if (online) {
      final response =
          await supabase.from(tableName).select().eq('userId', user.id).order(
    ...
    if (kIsWeb) {
      throw const AuthException('Sem conexão para consultar dados no Web');
    }
    ...
    final rows = await db.query(
      ...
      orderBy: '${orderBy ?? defaultOrderBy} '
          '${(ascending ?? defaultAscending) ? 'ASC' : 'DESC'}',
    );
    return rows.map(fromMap).toList();
  }
  ...
  Future<void> excluir(String id) async {
    ...
    if (kIsWeb) {
      throw const AuthException('Sem conexão para excluir dados no Web');
    }

    await _deleteLocal(id);
    await offlineSyncService.enqueueDelete(
      tableName: tableName,
      recordId: id,
  ...
  Future<void> _saveLocal(
    ...
    if (kIsWeb) {
      if (enqueue) {
        throw const AuthException('Sem conexão para salvar dados no Web');
      }
      return;
    }
