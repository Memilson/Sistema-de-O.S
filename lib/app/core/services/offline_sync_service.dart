...
class OfflineSyncService {
    ...
    for (final row in rows) {
      ...
      try {
        if (operation == 'delete') {
          ...
          await db.insert(tableName, syncPayload, conflictAlgorithm: ConflictAlgorithm.replace);
        }
  ...
  Future<Map<String, dynamic>> _uploadEmbeddedEvidences({required String tableName, required String recordId, required Map<String, dynamic> payload}) async {
    ...
    if (_isDataUrl(fotoAntesPath)) {
      syncedPayload['fotoAntesPath'] = await _uploadDataUrl(storage: storage, ordemId: recordId, fileName: 'foto_antes.jpg', dataUrl: fotoAntesPath!);
    }
    if (_isDataUrl(fotoDepoisPath)) {
      syncedPayload['fotoDepoisPath'] = await _uploadDataUrl(storage: storage, ordemId: recordId, fileName: 'foto_depois.jpg', dataUrl: fotoDepoisPath!);
    }
    if (_isDataUrl(assinaturaBase64)) {
      syncedPayload['assinaturaBase64'] = await _uploadDataUrl(storage: storage, ordemId: recordId, fileName: 'assinatura.png', dataUrl: assinaturaBase64!);
    }
